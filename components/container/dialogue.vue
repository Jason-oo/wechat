<template>
	<div>
		<xheader :name="nickname" leftTitle="微信" isLeftTitleShow="true" isRightTitleShow="true" isAddBtnShow="" leftHref="#/tab/wechat"></xheader>
		<ul class="chatlist">
			<li v-for="a in arr">
...
<script>
	...
	export default {
		...
		methods: {
			...
			sendMessage() {
				let self = this;
				this.$store.state.socket.emit("sendMsg", {
					msg: this.smsg
				})
				this.arr.push({
					msg: self.smsg,
				...
				this.smsg = "";
				this.$nextTick(() => {
					document.querySelector(".chatlist").scroll(0, 10000);
				})
			}
		},
		components: {
		...
		mounted() {
			let self = this;
			this.$store.state.tid = this.$route.params.id;
			this.ownPortrait = this.$store.state.user.portrait;
			this.$store.state.socket.on("receiveMsg", function(data) {
				self.arr.push({
					msg: data.msg,
					type: 1
				});
			...
			$.ajax({
				type: "get",
				url: "http://10.40.153.58:7878/getUserInfo?tid=" + self.$route.params.id,
				success(data) {
					console.log(data);
